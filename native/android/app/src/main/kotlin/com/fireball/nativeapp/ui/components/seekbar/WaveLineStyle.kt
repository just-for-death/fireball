package com.fireball.nativeapp.ui.components.seekbar
...
/**
 * Wave Line style seekbar — sine wave line that animates while playing.
 * Port of `app/.../ui/components/seekbar/WaveLineStyle.kt`. Replaces
 * `Math.PI` with `kotlin.math.PI` for commonMain.
 */
object WaveLineStyle {
    ...
    fun DrawScope.draw(
        ...
        val frequency = 0.08f
        val phase = (wavePhase * PI / 180.0).toFloat()
        ...
        val waveAmp = if (isPlaying) amplitude else amplitude * 0.2f
        val currentWaveY =
            centerY + sin(progressX * frequency + (if (isPlaying) phase else 0f)) * waveAmp
